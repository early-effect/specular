...
- **`docs` is unpublished dogfood.** DocSpecs live under `docs/src/main/scala` (shared JVM+JS Compile). `BuildSite` is `docs/src/main/scalajvm`. Thin `DocSpecSuite` wrappers and contract specs stay under `docs/src/test/scalajvm`. `ClientMain` is `docs/src/main/scalajs`.

## Architecture (the parts that span files)

**One AST, two interpreters.** [core/.../Doc.scala](core/src/main/scala/specular/Doc.scala) defines `DocPage` and the `DocNode` ADT: `Prose`, `Section`, and the example kinds (`Example` for UI samples, `Illustration` for a mount that is the document, `ValueExample` for plain values and effects, `FailExample` for must-not-compile snippets, `CrashExample` for must-fail effects, `DomExample` for a framework-agnostic browser mount). Two independent folds consume it:
...
**Docs live on Compile.** DocSpecs and the `DocsSite` subclass go under `src/main`. Thin `DocSpecSuite` wrappers go under `src/test`. [DocsSite](site/src/main/scala/specular/site/DocsSite.scala) reads fail-loud `-Dspecular.meta.*` via [ProjectMeta.fromSystemProperties](core/src/main/scala/specular/site/ProjectMeta.scala), and [SpecularPlugin](sbt-specular/src/main/scala/specular/sbt/SpecularPlugin.scala) forks the builder main with `(Test / fullClasspath)` (Test includes Compile). Set `specularMetaProject` to the **published** module, never the docs project, so the site advertises real coordinates.

**This repo cannot load its own plugin**, so [build.sbt](build.sbt) hand-rolls equivalent `specularSite` / `specularSiteDev` / `specularPreview` tasks in the `docs` matrix (`docsJS/spliceFull` for publish, `spliceFast` for `docs/specularPreview`, copy into `target/site/assets/client.js`, compile Test, fork `specular.docs.BuildSite` with the same `-D` props). The edit loop is `sbt docs/specularPreview` (no `~`): it delegates to `ascentPreview`, whose poller watches `ascentPreview / fileInputs` and queues rebuilds onto the live session. At an sbt prompt it returns; stop with `docs/specularPreviewStop`. Dogfood appends Test and `docsJS` Compile `fileInputs` the same way the plugin uses `specularJsProject`. **Changes to plugin behavior must be mirrored into those tasks**, or the dogfood site stops matching what consumers get.
...
- **`exampleZIO` accepts `ZIO[Scope, E, A]`.** `E` need not be `Nothing` or a `Throwable`. An unexpected typed `E` fails the site build (and an asserted test) with `E` in the report; do not fold `E` into `A` or `orDie` in the snippet. Defects (`die`) still fail the site/test.
- **`exampleError` is a documented typed failure.** Same `ValueExample` node; the result *is* `E`. `.assert` / `.withShow` take `E`. A body that succeeds is an `IllegalStateException` during the site build and an `assertTrue(false)` in tests. Defects stay defects, not a pretty `E`.
- **`expectCrash` must actually fail.** A body that succeeds is an `IllegalStateException` during the site build and an `assertTrue(false)` in tests. Its result is `Cause[E]` (`prettyPrint` by default). Use this for defects / `Throwable`; typed ADT errors belong on `exampleError`.
...
- **Nested sidebars derive from a product type.** `derives SiteNav` over a nested case class turns the site map into a `NavModel`, with `@navLabel("…")` overriding the humanized type name ([NavModel.scala](site/src/main/scala/specular/site/NavModel.scala), [SiteNavMacros.scala](site/src/main/scala/specular/site/SiteNavMacros.scala)). `NavModel.pages` is the depth-first flatten that drives routing and `metadata.json`. [BuildSite](docs/src/main/scalajvm/specular/docs/BuildSite.scala) is the worked example.
- **`illustration` / `illustrationIO` is a mount, not a sample.** No source panel, no `figure.specular-example`. `.live` remounts through `fromPages` like `.interactive`. Do not add a `.hideSource` flag on `example`; the figure chrome would still say "this is a sample."
- **Adding an ascent interactive example is a three-file change** in this repo: `.interactive` on the example (or `.live` on an illustration), the page listed in `ClientMain.pages` (and in `InteractiveContractSpec.clientPages`, which mirrors that list on the JVM), and a `DocSpecSuite` in [DocSuites.scala](docs/src/test/scalajvm/specular/docs/DocSuites.scala) for JVM discovery. `SpecularClient.fromPages` derives the mounter once the page is listed.
- **Adding an `exampleDom` example needs a mounter and a key.** Write the mount code in `docs/src/main/scalajs`, wrap the excerpt in `// specular:begin <marker>` / `// specular:end`, add the key to [InteractiveRegistry](docs/src/main/scala/specular/docs/InteractiveRegistry.scala) (single-sourced so the page, the JVM spec, and `ClientMain` cannot drift), and bind it in `ClientMain.extraMounters`. [RawDomDemo](docs/src/main/scalajs/specular/docs/RawDomDemo.scala) is the worked example, deliberately raw DOM: it proves the hook needs no UI library, which naming a framework would not.
