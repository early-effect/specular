...
**This repo cannot load its own plugin**, so [build.sbt](build.sbt) hand-rolls equivalent `specularSite` / `specularSiteDev` tasks in the `docs` matrix (`docsJS/spliceFast` for both until [sbt-splice#11](https://github.com/early-effect/sbt-splice/issues/11), copy into `target/site/assets/client.js`, compile Test, fork `specular.docs.BuildSite` with the same `-D` props). **Changes to plugin behavior must be mirrored into those tasks**, or the dogfood site stops matching what consumers get.
